...
Es va executar una comanda d’OpenSSL per generar una clau privada i un certificat SSL autofirmat vàlid durant 365 dies per al servidor. Durant el procés es van introduir dades d’identificació
...
Es va configurar un VirtualHost d’Apache que escolta al port 443 (HTTPS) per al domini projectenexus.test.
També es van definir el certificat SSL i la clau privada perquè el servidor pugui oferir connexions segures amb HTTP/2 i HTTP/1.1.
...
Ara fem el mateix que abans per ara ho farem amb academia.test

---

![imatge](/Tasca02/IMG/21.png)

Aquest fitxer configura un VirtualHost d’Apache per al domini projectenexus.test, definint el nom del servidor, àlies i la carpeta arrel del lloc web. També redirigeix totes les peticions HTTP cap a HTTPS i estableix els fitxers de registre d’errors, accessos i la pàgina d’error 404.
...
Ara farem el mateix pero amb academia.test
