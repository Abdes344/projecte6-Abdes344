# Nou Encàrrec: Infraestructura Web per a Nexus 🌐🚀
...
**Nexus** és una nova empresa de formació a Mataró que ens ha contactat per al desplegament i gestió de la seva infraestructura web.  

En aquesta fase inicial, l'objectiu és establir una base sòlida i segura per als seus serveis corporatius abans de migrar definitivament al núvol. ☁️  
...
Com a tècnics especialistes, heu de configurar el servidor web **Apache** sobre **Ubuntu Server** donant resposta a aquestes necessitats amb rigor professional. 🖥️  

---

# 📋 Descripció de l’encàrrec

Previ al desplegament en un VPS, desenvolupareu el projecte en una màquina virtual en local.  

Un cop superades les proves de funcionament i el client doni el vistiplau, ja es desplegarà sobre l’entorn a Internet. 🌍  

Les accions a realitzar consisteixen en la **instal·lació, configuració de dominis virtuals i securització avançada d'un servidor web Apache**.  

Heu de documentar tot el procés en l'informe tècnic del projecte. 📝  
...
- Activeu els llocs amb la comanda `a2ensite`.
- Modifiqueu l'arxiu `hosts` per simular la resolució de noms (DNS) i que els dominis responguin correctament. 🌍  

---

## 3️⃣ Personalització d'Errors

Configureu una pàgina d'error personalitzada pel codi **404 (Not Found)** per a, com a mínim, un dels VirtualHosts.  

El missatge ha de ser corporatiu i professional, evitant la pàgina per defecte del servidor. 📄  
...
- **Redirecció forçada:** Configureu el servidor perquè qualsevol petició HTTP (port 80) a `projectenexus.test` i `academia.test` es redirigeixi automàticament a HTTPS (port 443). 🔁  
...
Redacteu una **memòria tècnica** de la instal·lació i configuració, incloent les proves de funcionament.  

És important que aquesta memòria tècnica contingui les explicacions i no només captures de pantalla.  

Els vostres clients no són experts i necessiten poder entendre el que heu fet. 👨‍💼📘  

---  

💡 Recordeu: una infraestructura ben configurada no només funciona, sinó que transmet confiança, professionalitat i seguretat.
